...
export async function publishWaiting(session_id: string, offer: InterventionOffer): Promise<void> {
  await redis.publish(
    `canvas:stream:${session_id}`,
    JSON.stringify({ type: 'waiting', offer })
  )
  logger.info('[streaming:offer] published waiting', { session_id, offer_id: offer.id, seq: offer.seq })
}
